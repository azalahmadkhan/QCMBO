%function [bestsol,bestfitness,BestFitIter,S_population,F] = real_coded_GA(prob,lb,ub,Np,T)
%% Parameters
n_variables=5;
LB=-5.12*ones(1,n_variables); % The lower bound of the chromosomes
UB= 5.12*ones(1,n_variables); % The upper bound of the chromosomes
myobjfunc=@Sphere;% The Objective function handle
generation=100;   % Maximum Number of Iterations
...
BestFitIter(ii)= S_population(1,n_variables+1);
end
plot(BestFitIter,'b');
pause(0.1)
xlabel('Generation')
ylabel('Best Value')
grid on
bestsol=S_population(1,1:n_variables);
bestfitness=BestFitIter(1,generation);
%end
